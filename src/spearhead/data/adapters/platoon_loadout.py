...
from spearhead.data.dto import TabularRecord


class PlatoonLoadoutAdapter:
    """
    Parses the platoon-level loadout & ammo sheet (דוחות פלוגת כפיר).
    The sheet has two side-by-side sections:
    - Left: דוח זיווד (equipment)
    - Right: תחמושת (ammo), positioned where row 1 contains the label "תחמושת".
    Column headers are on row 2; data starts on row 3.
    """
    ...
    def load(cls, file_path: Path) -> List[TabularRecord]:
        ...
        for idx, row in enumerate(rows[2:], start=3):
            # Stop only when the row is entirely empty
    ...
    def _collect_row(
        ...
        for j, header in enumerate(headers):
            ...
            if not header_text:
                continue
            if header_text in ("הפריט", "תקן", "סהכ פלוגתי"):
                continue
            col_idx = offset + j
